...
const JIRA_WRITEBACK_STAGE = 'Artifact Linking';
...
export const createJiraWritebackPreviewState = ({
  context,
  draftRef,
  draft,
  updatedAt,
  ...
  const previousApprovalId = context.active_approval_ref_map[JIRA_WRITEBACK_STAGE];
  const nextApprovalMap = {
    ...context.active_approval_ref_map,
  };

  delete nextApprovalMap[JIRA_WRITEBACK_STAGE];
  ...
  return {
    context: {
      ...
      jira_writeback_draft_ref: draftRef,
      jira_writeback_result_ref: null,
      stage_status_map: {
        ...context.stage_status_map,
        [JIRA_WRITEBACK_STAGE]: 'output_ready',
      },
      stage_artifact_refs: {
        ...context.stage_artifact_refs,
        [JIRA_WRITEBACK_STAGE]: [draftRef],
      },
...
export const buildJiraWritebackApprovalRecord = ({
  approvalId,
  decider,
  previewRef,
  previewHash,
...
export const guardJiraWritebackRequirementBinding = ({
  projectProfile,
  context,
  timestamp,
}: {
  ...
  return StructuredErrorSchema.parse({
    ...
    stage: JIRA_WRITEBACK_STAGE,
    system: 'workflow',
    operation: 'guard-jira-writeback',
    target_ref: context.jira_issue_snapshot_ref,
    message:
      'Requirement binding must be resolved before Jira writeback can execute for this project.',
    detail:
    ...
    user_action:
      'Resolve the requirement binding or relax the project policy before retrying Jira writeback.',
    raw_cause_ref: null,
...
export const buildJiraWritebackPreparedEntry = ({
  draft,
  attemptNo,
  executedAt,
}: {
  ...
  SideEffectLedgerEntrySchema.parse({
    system: 'jira',
    operation:
      draft.target_type === 'comment' ? 'write-comment' : 'write-field',
    idempotency_key: draft.idempotency_key,
...
export const markJiraWritebackEntryDispatched = ({
  entry,
  externalRequestId,
  executedAt,
}: {
...
export const finalizeJiraWritebackEntry = ({
  entry,
  resultRef,
  result,
  executedAt,
...
export const shouldSkipJiraWritebackExecution = ({
  dryRun,
  latestEntry,
}: {
  dryRun: boolean;
